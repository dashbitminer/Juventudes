    ...
    <x-popover.panel class="z-10 w-64 overflow-hidden border border-gray-100 shadow-xl">
        <div class="flex flex-col divide-y divide-gray-100">
            <label class="flex items-center gap-2 px-3 py-2 cursor-pointer hover:bg-gray-100">
                <input value="-1" wire:model.live="selectedCohortesIds"
                type="checkbox" class="border-gray-300 rounded">
                <div class="text-sm text-gray-800">Seleccionar Todo</div>
            </label>
            @foreach ($cohortes as $cohorte)
                <label class="flex items-center gap-2 px-3 py-2 cursor-pointer hover:bg-gray-100">
                    <input value="{{ $cohorte->id }}"
                    wire:model.live="selectedCohortesIds"
                    type="checkbox" class="border-gray-300 rounded"
                    >
                    <div class="text-sm text-gray-800">{{ $cohorte->cohorte->nombre ?? "" }} </div>
                </label>
            @endforeach
        </div>
    </x-popover.panel>
