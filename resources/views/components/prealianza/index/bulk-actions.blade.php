<div class="flex flex-col col-span-5 gap-2 sm:flex-row sm:justify-end">
    <div class="flex flex-row-reverse justify-end gap-2 sm:justify-start sm:flex-row" x-show="$wire.selectedPreAlianzasIds.length > 0" x-cloak>
        <div class="flex items-center gap-1 text-sm text-gray-600">
            <span x-text="$wire.selectedPreAlianzasIds.length"></span>

            <span x-text="$wire.selectedPreAlianzasIds.length == 1 ? 'seleccionado' : 'seleccionados'"></span>
        </div>
        ...
        @if(auth()->user()->can('Eliminar Pre Alianza'))
    ...
    <div class="hidden sm:flex">
        @if(auth()->user()->can('Exportar Pre Alianza'))
