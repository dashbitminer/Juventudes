<div class="flex flex-col col-span-5 gap-2 sm:flex-row sm:justify-end">
    <div class="flex flex-row-reverse justify-end gap-2 sm:justify-start sm:flex-row" x-show="$wire.selectedParticipanteIds.length > 0" x-cloak>
        ...
        <div class="flex items-center px-3">
            ...
            <button type="submit" wire:click="previewSelectedMover" class="flex items-center gap-2 rounded-lg border px-3 py-1.5 bg-white font-medium text-sm text-gray-700 hover:bg-gray-200 disabled:cursor-not-allowed disabled:opacity-75">
                <x-icon.archive-box wire:loading.remove wire:target="previewSelectedMover" />

                <x-icon.spinner wire:loading wire:target="previewSelectedMover" class="text-gray-700" />
            ...
            >
                <x-icon.receipt-refund wire:loading.remove wire:target="previewSelected" />

                <x-icon.spinner wire:loading wire:target="previewSelected" class="text-white" />
