...
@props(['periodo_proyectos', 'proyectos'])
...
<div x-data="{ activeTab: 0 }">
    <div class="grid grid-cols-1 gap-6 mt-8 md:grid-cols-2 lg:grid-cols-4" >
        ...
        <div x-on:click="activeTab = 4"
            class="block max-w-xs p-6 mx-auto space-y-3 rounded-lg shadow-lg cursor-pointer group ring-1 ring-slate-900/5 hover:bg-indigo-500 hover:ring-indigo-500"
            :class="{'hover:bg-white-500 hover:ring-white-500 bg-indigo-500 ring-indigo-500': activeTab === 4, 'hover:bg-indigo-500 bg-white hover:ring-indigo-500': !(activeTab === 4 )}">
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    <svg class="w-6 h-6 stroke-indigo-500 group-hover:stroke-white"
                        :class="{'stroke-white': activeTab === 4, 'group-hover:stroke-white': !(activeTab === 4 )}"
                        fill="none" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M11 19H6.931A1.922 1.922 0 015 17.087V8h12.069C18.135 8 19 8.857 19 9.913V11"></path>
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <h3 class="text-sm font-semibold text-slate-900 group-hover:text-white"
                        :class="{'text-white': activeTab === 4, 'text-slate-900': !(activeTab === 4 )}">
                        Estipendios
                    </h3>
                </div>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor" class="w-6 h-6 text-indigo-500 group-hover:stroke-white"
                    :class="{'text-white': activeTab === 4, 'text-indigo-500': !(activeTab === 4 )}">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                </svg>
            </div>
            <p class="text-sm text-slate-500 group-hover:text-white"
                :class="{'text-white': activeTab === 4, 'text-slate-500': !(activeTab === 4 )}">
                Estipendios: Gestión de pagos, reportes, etc.
            </p>
        </div>
    ...
    <div class="mt-4" x-cloak>
        <div x-show="activeTab == 3" class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3"
            ...
            @if(auth()->user()->can('Ver mis participantes R4'))
                ...
                <div class="p-4 bg-white rounded-lg shadow-md">
                    <h2 class="text-xl font-semibold">
                        ...
                        <h2 class="text-xl font-semibold">Participantes - {{ $proyecto->cohorte_nombre }}</h2>
                    </a>
                    </h2>
                    <div class="flex items-center gap-2 text-sm font-medium text-green-500">
                        Activo
                    ...
                    <p class="mt-1 text-sm text-gray-500">
                        Agregar ficha de enlace a empleo, educativa, emprendimiento, etc. a Participantes
                    </p>
                </div>
                @endforeach
            ...
            @elseif(
                 auth()->user()->can('Ver participantes mi socio implementador R4') ||
                 auth()->user()->can('Ver participantes mi pais R4') ||
                 auth()->user()->can('Filtrar registros por socios por pais'
             ))
                ...
                <div class="p-4 bg-white rounded-lg shadow-md">
                    <h2 class="text-xl font-semibold">
                        ...
                        <h2 class="text-xl font-semibold">{{ $proyecto->proyecto_nombre }}</h2>
                    </a>
                    </h2>
                    <div class="flex items-center gap-2 text-sm font-medium text-green-500">
                        Activo
    ...
    <div class="mt-4" x-cloak>
        <div x-show="activeTab == 4" class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3"
            x-show.transition.in.opacity.duration.600="activeTab === 4">

            {{-- @if(auth()->user()->can('Ver estipendios')) --}}
                @foreach($proyectos as $proyecto)
                    <div class="p-4 bg-white rounded-lg shadow-md">
                        <h2 class="text-xl font-semibold">
                            <a wire:navigate
                            href="/pais/{{ $proyecto->pais_slug }}/proyecto/{{ $proyecto->proyecto_slug }}/cohorte/{{ $proyecto->cohorte_slug }}/estipendios">
                            {{ $proyecto->proyecto_nombre }}
                            </a>
                        </h2>
                        <div class="flex items-center gap-2 text-sm font-medium text-green-500">
                        ...
                        <p class="font-bold text-black">{{ $proyecto->cohorte_nombre }}</p>
                        <p class="text-gray-600">{{ $proyecto->pais_nombre }}</p>
                        <p class="text-gray-600">Gestión de pagos y reportes</p>
                    </div>
                @endforeach
            {{-- @endif --}}
