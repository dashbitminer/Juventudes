<div class="flex flex-col col-span-5 gap-2 sm:flex-row sm:justify-end">
    <div class="flex flex-row-reverse justify-end gap-2 sm:justify-start sm:flex-row" x-show="$wire.selectedAlianzasIds.length > 0" x-cloak>
        <div class="flex items-center gap-1 text-sm text-gray-600">
            <span x-text="$wire.selectedAlianzasIds.length"></span>

            <span x-text="$wire.selectedAlianzasIds.length == 1 ? 'seleccionado' : 'seleccionados'"></span>
        </div>
    ...
    <div class="hidden sm:flex">
        @if(auth()->user()->can('Exportar Alianza'))
