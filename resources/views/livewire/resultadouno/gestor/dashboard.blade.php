...
<div>
    ...
    @if(auth()->user()->hasRole("Financiero"))
        <x-dashboard.financiero :$proyectos/>
    @elseif(auth()->user()->hasRole("Validación R4"))
        <x-dashboard.validadorr4 :$proyectos/>
    @elseif(auth()->user()->hasRole("MECLA"))
        <x-dashboard.mecla :$proyectos :$periodo_proyectos/>
    @else

        <div x-data="{ activeTab: 0 }">
            <div class="grid grid-cols-1 gap-6 mt-8 md:grid-cols-2 lg:grid-cols-4" >
                @if(!auth()->user()->hasAnyRole(['Registro R3', 'Validación R3']))
            ...
            @if(!auth()->user()->hasAnyRole(['Registro R3', 'Validación R3', 'Validación R4']))
            <div class="mt-4">
