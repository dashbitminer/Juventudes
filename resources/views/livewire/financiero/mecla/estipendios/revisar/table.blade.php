    ...
    <div class="flow-root mt-8">
        ...
        <div class="flex flex-col grid-cols-8 gap-2 my-4 sm:grid">
            ...
            <div class="flex col-span-5 gap-2 sm:justify-end">
                <x-estipendios.filter-estados :$estados />
                <x-estipendios.bulk-actions-revisar :$estipendio />
            </div>
        </div>
                            ...
                            <table class="min-w-full divide-y divide-gray-300">
                                <thead class="sticky top-0 z-20 bg-gray-50">
                                    <tr>
                                        ...
                                        <th scope="col"
                                            class="sticky left-0 z-20 py-3.5 pr-3 pl-4 text-sm font-semibold text-left text-gray-900 bg-gray-50 sm:pl-0">
                                        ...
                                        <th scope="col" class="px-3 py-3.5 text-sm font-semibold text-left text-white text-nowrap"
                                            style="background-color: {{ $agrupacion['color'] }}">
                                            <x-participante.index.sortable column="agrupacion_{{ $agrupacion['id'] }}" :$sortCol :$sortAsc>
                                                % {{ $agrupacion["nombre"] }}
                                            </x-participante.index.sortable>
                                        </th>
                                        ...
                                        <th scope="col" class="px-3 py-3.5 text-sm font-semibold text-left text-gray-900">
                                            <div>Promedio de cumplimiento</div>
                                        </th>
                                        <th scope="col" class="px-3 py-3.5 text-sm font-semibold text-left text-gray-900">
                                            <div class="whitespace-nowrap">Alerta general</div>
                                        </th>
                                        <th scope="col" class="px-3 py-3.5 text-sm font-semibold text-left text-gray-900">
                                            <div>Monto de Estipendio</div>
                                        </th>
                                        <th scope="col" class="px-3 py-3.5 text-sm font-semibold text-left text-gray-900">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    ...
                                    <tr wire:key='rand-{{ $participante->id }}'
                                        ...
                                        <td class="relative sticky left-0 z-10 p-0 text-sm bg-white sm:pl-0"  :class="isSelected ? 'bg-gray-50' : 'bg-white'">
                                            ...
                                            <div class="w-[300px] py-5 pr-3 pl-4 flex items-center">
                                                <div class="ml-4">
                                        ...
                                        <td class="px-3 py-5 text-sm text-gray-500 whitespace-nowrap">
                                            @if ($participante->grupoactivo)
                                                @if ($participante->grupoactivo->lastEstadoParticipante->fecha)
                                                    {{ \Carbon\Carbon::parse($participante->grupoactivo->lastEstadoParticipante->fecha)->format('M d, Y') }}
                                                @else
                                                    @if ($participante->grupoactivo->lastEstadoParticipante->estado->nombre != 'Activo')
                                                        No disponible
                                                    @endif
                                                @endif
                                            @else
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">${{ $participante->monto_estipendio ?? '0.0' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="w-24">
                                                @if ($estipendio->is_closed == 0)
                                                    <flux:input class="w-full" class:input="font-mono" type="number" min="0" max="100"
                                                        wire:blur="updateParticipantePorcentaje({{ $participante->id }}, $event.target.value)"
                                                        value="{{ $participante->porcentaje_estipendio ?? '0.0' }}" />
                                                @else
                                                    <flux:input class="w-full" class:input="font-mono" disabled
                                                        value="{{ $participante->porcentaje_estipendio ?? '0.0' }}" />
                                                @endif
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="w-64">
                                                @if ($estipendio->is_closed == 0)
                                                    <flux:textarea rows="2" resize="none"
                                                        wire:blur="updateParticipanteObservacion({{ $participante->id }}, $event.target.value)">
                                                        {{ $participante->observacion }}
                                                    </flux:textarea>
                                                @else
                                                    <flux:textarea rows="2" resize="none" disabled>
                                                        {{ $participante->observacion }}
                                                    </flux:textarea>
                                                @endif
                                            </div>
