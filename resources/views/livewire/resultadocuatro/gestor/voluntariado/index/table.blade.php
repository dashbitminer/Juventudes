    ...
    {{-- <div class="-mx-4 -my-2 overflow-x-auto sm:-mx-6 lg:-mx-8"> --}}
                ...
                <div class="relative">
                    <table class="min-w-full divide-y divide-gray-300">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr wire:key='rand-{{ $voluntariado->id }}'>
                                <td class="p-3 text-sm whitespace-nowrap">
                                    <div class="flex items-center">
                                        <input wire:model="selectedAlianzasIds" value="{{ $voluntariado->id }}"
                                ...
                                <td class="py-5 pl-4 pr-3 text-sm whitespace-nowrap sm:pl-0">
                                    <div class="flex items-center">
                                        <div class="ml-4">
                                            <div class="font-medium text-gray-900">{{ $voluntariado->nombre_organizacion }}
                                            </div>
                                            <div class="mt-1 text-gray-500">{{ $voluntariado->telefono_contacto }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-3 py-5 text-sm text-gray-500 whitespace-nowrap">
                                    <div class="text-gray-900">{{ $voluntariado->nombre_contacto }}</div>
                                    <div class="mt-1 text-gray-500">{{ $voluntariado->email_contacto }}</div>
                                </td>
                                <td class="px-3 py-5 text-sm text-gray-500 whitespace-nowrap">
                                    <div class="text-gray-900">{{ $voluntariado->ciudad->nombre }}</div>
                                    <div class="mt-1 text-gray-500">{{ $voluntariado->ciudad->departamento->nombre }}</div>
                                ...
                                <td>
                                    {{ $voluntariado->created_at->format('d/m/Y g:i A') }}
                                </td>
                                <td
        ...
        <div class="flex items-center justify-between pt-4">
            <div class="text-sm text-gray-700">
                @php
                $start = ($voluntariados->currentPage() - 1) * $voluntariados->perPage() + 1;
                $end = min($voluntariados->currentPage() * $voluntariados->perPage(), $voluntariados->total());
                @endphp
                Mostrando {{ $start }} a {{ $end }} de un total de {{
                \Illuminate\Support\Number::format($voluntariados->total()) }} registros
            </div>

            <div class="text-sm text-gray-700">
                Página actual: {{ $voluntariados->currentPage() }} de {{ $voluntariados->lastPage() }}
            </div>

            {{ $voluntariados->links('livewire.resultadouno.gestor.participante.index.pagination') }}
        </div>
