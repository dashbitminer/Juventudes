                                ...
                                @foreach ($estados as $estado)
                                    <tr wire:key="{{ $estado->id }}" class="odd:bg-white even:bg-slate-50">
                                        <td class="px-3 py-5 text-sm text-gray-500 whitespace-nowrap">
                                            <div class="rounded-full py-1 pl-2 pr-1 inline-flex font-medium items-center gap-1 text-{{ Estados::from($estado->estado)->color() }}-600 text-xs bg-{{ Estados::from($estado->estado)->color() }}-100">
                                                <div>{{ Estados::from($estado->estado)->label() }}</div>
                                                <x-dynamic-component :component="Estados::from($estado->estado)->icon()" />
                                            </div>
                                        </td>
                                        <td class="px-3 py-5 text-sm text-gray-500 whitespace-nowrap">
                                            {{ $estado->comentario }}
