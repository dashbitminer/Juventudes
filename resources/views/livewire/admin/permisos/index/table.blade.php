<div class="flow-root mt-8">
                ...
                <table class="min-w-full divide-y divide-gray-300">
                    <tr>
                        ...
                        <th scope="col" class="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900 sm:pl-0">
                            <x-participante.index.sortable column="permisos" :$sortCol :$sortAsc>
                                <div class="whitespace-nowrap">Permiso</div>
                            </x-participante.index.sortable>
                        </th>
                        <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">
                            <x-participante.index.sortable column="categoria" :$sortCol :$sortAsc>
                                Categoria
                            </x-participante.index.sortable>
                        </th>
                        <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr wire:key='rand-{{ $permiso->id }}'>
                            <td class="p-3 text-sm whitespace-nowrap">
                                <div class="flex items-center">
                                    <input wire:model="selectedIds" value="{{ $permiso->id }}"
                            ...
                            <td class="py-5 pl-4 pr-3 text-sm whitespace-nowrap sm:pl-0">
                                <div class="flex items-center">
                                    <div class="ml-4">
                                        <div class="font-medium text-gray-900">{{ $permiso->name }}</div>
                                        <div class="mt-1 text-gray-500">Roles asignados: {{ $permiso->roles_count }}</div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-3 py-5 text-sm text-gray-500 whitespace-nowrap">
                                {{ $permiso->creator->name ?? "System" }}</td>

                            <td class="p-3 text-sm whitespace-nowrap">
                                {{ $permiso->dateForHumans() }}
                            </td>

                            <td class="relative py-5 pl-3 pr-4 text-sm font-medium text-right whitespace-nowrap sm:pr-0">
                                <x-admin.permisos.user-row-dropdown :$permiso wire:key='permiso-{{ $permiso->id}}' />
                            </td>
                        </tr>
    ...
    <div class="flex items-center justify-between pt-4">
        <div class="text-sm text-gray-700">
            @php
            $start = ($permisos->currentPage() - 1) * $permisos->perPage() + 1;
            $end = min($permisos->currentPage() * $permisos->perPage(), $permisos->total());
            @endphp
            Mostrando {{ $start }} a {{ $end }} de un total de {{
            \Illuminate\Support\Number::format($permisos->total()) }} registros
        </div>

        <div class="text-sm text-gray-700">
            Página actual: {{ $permisos->currentPage() }} de {{ $permisos->lastPage() }}
        </div>

        {{ $permisos->links('livewire.resultadouno.gestor.participante.index.pagination') }}
    </div>
